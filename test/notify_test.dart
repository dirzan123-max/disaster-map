...
void main() {
  group('通知条件', () {
    test('深刻度が下限未満なら通知しない', () {
      const settings = NotificationSettings(minimumSeverity: Severity.severe);
      expect(settings.matches(event(severity: Severity.extreme)), isTrue);
      expect(settings.matches(event(severity: Severity.severe)), isTrue);
      expect(settings.matches(event(severity: Severity.moderate)), isFalse);
    });
    ...
    test('対象外の種別は通知しない', () {
      ...
      expect(settings.matches(event(kind: EventKind.earthquake)), isTrue);
      expect(settings.matches(event(kind: EventKind.wildfire)), isFalse);
    });

    test('通知を切っていれば何も通らない', () {
      const settings =
          NotificationSettings(enabled: false, minimumSeverity: Severity.info);
      expect(settings.matches(event(severity: Severity.extreme)), isFalse);
    });

    test('保存して読み直しても条件が変わらない', () {
      const original = NotificationSettings(
        minimumSeverity: Severity.moderate,
        kinds: {EventKind.tsunami, EventKind.volcano},
        webhookUrl: 'https://example.com/hook',
        webhookFormat: WebhookFormat.teams,
      );
      final restored =
          NotificationSettings.fromJson(jsonDecode(jsonEncode(original.toJson())));
      expect(restored.minimumSeverity, original.minimumSeverity);
